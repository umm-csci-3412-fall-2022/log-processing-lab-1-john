...
cat "$targetDirectory"/var/log/* | sed -n -E 's/^([A-Za-z]* [0-9]{2}) ([0-9]{2}):[0-9]{2}:[0-9]{2} [\S]* [A-Za-z]+\[[0-9]*\]: Failed password for (invalid user )?(\S*) from ([0-9\.]+) port [0-9]* ssh2$/\1 \2 \4 \5/p' > "$targetDirectory"/failed_login_data.txt
