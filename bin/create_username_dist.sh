...
sort "$tempFile" | uniq -c | sed -n -E "s/^[ ]+([0-9]+) ([A-Za-z0-9_-]*)$/data.addRow(['\2', \1]);/p" | bin/wrap_contents.sh /dev/stdin html_components/username_dist "$inputDirectory"/username_dist.html
